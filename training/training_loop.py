...
def training_loop(
    ...
    if dist.get_rank() == 0:
        with torch.no_grad():
            images = torch.zeros(
                [batch_gpu, img_channels, net.img_resolution, net.img_resolution],
                device=device,
            )
            sigma = torch.ones([batch_gpu], device=device)
            x_pos = torch.zeros(
                [batch_gpu, 2, net.img_resolution, net.img_resolution], device=device
            )
            labels = torch.zeros([batch_gpu, net.label_dim], device=device)
            misc.print_module_summary(
                net, [images, sigma, x_pos, labels], max_nesting=2
            )
    ...
    stats_jsonl = None
    batch_mul_dict = {512: 1, 256: 2, 128: 4, 64: 16, 32: 32, 16: 64, 8: 128, 4: 256}
    if train_on_latents:
        ...
        patch_list = np.array(
            [img_resolution // 4, img_resolution // 2, img_resolution]
        )
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp, (round_idx == num_accumulation_rounds - 1)):
                if progressive:
                    ...
                    batch_mul_avg = (
                        batch_mul_dict[patch_size] // batch_mul_dict[img_resolution]
                    )
                else:
